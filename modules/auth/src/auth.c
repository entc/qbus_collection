...
static int __STDCALL qbus_auth_ui_merge (QBus qbus, void* ptr, QBusM qin, QBusM qout, CapeErr err)
{
  AuthContext ctx = ptr;
  ...
  // run the command
  return auth_ui_merge (&auth_ui, qin, qout, err);
}
...
static int __STDCALL qbus_auth_ui_users (QBus qbus, void* ptr, QBusM qin, QBusM qout, CapeErr err)
{
  AuthContext ctx = ptr;
...
int auth_context_init (AuthContext self, QBus qbus, CapeErr err)
{
  ...
  self->adbl_session = adbl_session_open_file (self->adbl_ctx, qbus_config_s(qbus_config (qbus), "adbl_config", "adbl_default.json"), err);
  ...
  self->options_2factor = qbus_config_node (qbus_config (qbus), "options_2factor");
  self->options_fp = qbus_config_node (qbus_config (qbus), "options_fp");
...
static int __STDCALL qbus_auth_init (QBus qbus, void* ptr, void** p_ptr, CapeErr err)
{
  ...
  *p_ptr = ctx;
  ...
  // merge user
  //   args: username, password
  qbus_register (qbus, "ui_merge"             , ctx, qbus_auth_ui_merge, NULL, err);
